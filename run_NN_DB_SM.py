...
ADAPTIVE_C = -5
CLOSING_RADIUS = 3
MANUAL_ERODE_RADIUS = 1 # [Modified] 腐蚀半径 (原7太大了导致粒子消失，建议3-4)
MANUAL_DILATE_RADIUS = 1  # [Modified] 膨胀半径 (建议比腐蚀小1-2，以保持平滑效果)
PRE_CLOSING_MIN_AREA = 100
MIN_OBJECT_AREA = 30000   # [Modified] 降低最小面积阈值，防止腐蚀后粒子变小被过滤
MAX_OBJECT_AREA = 180000
MAX_OBJECT_AREA_FINAL = 250000 # [New] 最终Mask的面积上限，用于剔除粘连后超标的粒子
MIN_OBJECT_AREA_FINAL = 60000  # [Modified] 修正：降低最终面积下限，防止误删有效粒子
FILL_HOLES = True
MIN_CIRCULARITY = 0.6
MAX_ASPECT_RATIO = 1.4
WATERSHED_MIN_DIST = 15
HOUGH_FIT_CIRCLE = True # Set to True to enable Hough circle repair for broken particles
...
def extract_structure_adaptive(image_path, save_prefix=None, hough_fit_circle=True):
    ...
    # [New] Add Gaussian Blur to smooth edges before thresholding
    ...
    # [Modified] Manual Erode -> Dilate strategy (Asymmetric Opening)
    # 1. Erode: Remove noise and jagged edges
    k_erode = MANUAL_ERODE_RADIUS * 2 + 1
    ...
    # 2. Dilate: Restore shape partially, keeping it smooth
    k_dilate = MANUAL_DILATE_RADIUS * 2 + 1
    dilate_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k_dilate, k_dilate))
    mask = cv2.dilate(eroded, dilate_kernel, iterations=1) > 0
    ...
    # [New] 最终质检：剔除面积不符合要求的粒子 (过大或过小)
    if MAX_OBJECT_AREA_FINAL > 0 or MIN_OBJECT_AREA_FINAL > 0:
        ...
        for r in regions_final:
            if (MAX_OBJECT_AREA_FINAL > 0 and r.area > MAX_OBJECT_AREA_FINAL) or \
               (MIN_OBJECT_AREA_FINAL > 0 and r.area < MIN_OBJECT_AREA_FINAL):
                final_mask[labels_final == r.label] = 0
                removed_final += 1
        
        if removed_final > 0:
            print(f"  [Structure] Removed {removed_final} particles outside final area range ({MIN_OBJECT_AREA_FINAL}-{MAX_OBJECT_AREA_FINAL}) from final mask.")

    final_count = kept_count - removed_final if 'removed_final' in locals() else kept_count
    print(f"  [Structure] Final count: {final_count} (Initial: {kept_count}, Removed: {removed_final if 'removed_final' in locals() else 0})")
...
def process_pair(ch00_path, ch01_path):
    ...
    try:
        ...
        brightness_threshold_value = int(0.6 * 255) # [Modified] 降低亮度阈值 (0.2 -> 0.1) 以检测较暗信号
